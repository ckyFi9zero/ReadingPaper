...
更新日期：2026-09-18。范围：ReadingPaper 仓库及本次会话已确认的项目需求。
...
- 纯静态网站已在本地构建，共 11 篇论文、46 个 HTML 页面。
...
- 目前领域内 9 篇（含扬尘工况）、领域外 2 篇。用户要求将 DHE-Net 移出领域内；按照明确的天气噪声边界，电弧噪声论文也归为领域外。两篇仍保留在论文库，不是删除论文或笔记。
- DHE-Net、DSOR 已有独立的第一遍初读 HTML，保留原有图像、页码引用及阅读交互。
- 用户指出现有第二遍深读不正确，2026-09-17 已撤下另外 7 篇旧 Paper Card 及其衍生跨论文综述，等待重新整理。
- 当前登记数量为初读 4 篇、深读 1 篇、代码阅读 0 篇。DHE-Net 深读来自用户提供的 `writing-close-reading.html`，聚焦写作结构与语言；没有在接入中复核其学术结论，也不表示完成公式推导、实验审计或代码复现。

...
if (search) {
  ...
  function update() {
    ...
    for (const card of cards) {
      card.hidden = !terms.every(term => card.dataset.search.includes(term)) || (stage.value !== "all" && !card.dataset.stages.split(" ").includes(stage.value));
  ...
  document.querySelector("#reset").addEventListener("click", () => { search.value = ""; stage.value = "all"; update(); search.focus(); });
  update();
}
